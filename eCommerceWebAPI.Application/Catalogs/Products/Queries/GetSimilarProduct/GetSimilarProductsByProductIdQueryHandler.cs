...
{
    public class GetSimilarProductsByProductIdQueryHandler : IQueryHandler<GetSimilarProductsByProductIdQuery, IList<ProductDto>>
    {
        private readonly ISqlConnectionFactory _sqlConnectionFactory;
        private readonly IMapper _mapper;

        public GetSimilarProductsByProductIdQueryHandler(ISqlConnectionFactory sqlConnectionFactory,
            IMapper mapper)
        {
        ...
        public async Task<IList<ProductDto>> Handle(GetSimilarProductsByProductIdQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                const string sqlSimilarProducts = $@"
                                    WITH RequestedProduct AS
                                        (
                                            SELECT TOP (1)
                                                p.Price - p.DiscountAmount AS CurrentProductAvailablePrice
                                            FROM [Product] p
                                    ...
                                    SELECT DISTINCT
                                       ...
                                       [Product].[Currency]
                                   FROM [Product]
                                   INNER JOIN [ProductCategoryMapping] pcm ON [Product].Id = pcm.ProductId
                                   WHERE
                                       [Product].Deleted = 0 AND
                                       [Product].StockQuantity > 0 AND
                                       [Product].Id <> @CurrentProductId AND
                                       pcm.CategoryId IN
                                           (
                                               SELECT [Category].Id
                                               FROM [Category]
                                               WHERE [Category].Id IN
                                                   (
                                                       SELECT pcm2.CategoryId
                                                       FROM [ProductCategoryMapping] pcm2
                                                       WHERE pcm2.ProductId = @CurrentProductId
                                                   )
                                           ) AND
                                       ([Product].Price - [Product].DiscountAmount) >= (SELECT CurrentProductAvailablePrice FROM RequestedProduct) * 0.9 AND
                                       ([Product].Price - [Product].DiscountAmount) <= (SELECT CurrentProductAvailablePrice FROM RequestedProduct) * 1.1
                                   ORDER BY [Product].Id
                                   OFFSET @Skip ROWS
                                   FETCH NEXT @PageSize ROWS ONLY";
                var productsEnumerable = await connection.QueryAsync<Product>(sqlSimilarProducts, new { CurrentProductId = request.ProductId, request.Skip, request.PageSize });
